...
ssh-keygen -t ed25519 -C "andreas.rau93@gmail.com" -f "${HOME}/.ssh/id_github"
eval "$(ssh-agent -s)"
echo "Host github.com \n AddKeysToAgent yes\n UseKeychain yes\n HostName github.com\n IdentityFile ~/.ssh/id_github\n" >> "${HOME}/.ssh/config"
ssh-add --apple-use-keychain "${HOME}/.ssh/id_github"
echo "run 'pbcopy < ~/.ssh/id_github.pub' and paste that into GitHub"

# SSH for  git.daimler.com/
echo "Generating a new SSH key for Enterprise GitHub"
ssh-keygen -t ed25519 -C "andreas.rau@mercedes-benz.io" -f "${HOME}/.ssh/id_enterprise_github"
eval "$(ssh-agent -s)"
echo "Host git.daimler.com \n AddKeysToAgent yes\n UseKeychain yes\n HostName git.daimler.com\n IdentityFile ~/.ssh/id_enterprise_github\n" >> "${HOME}/.ssh/config"
ssh-add --apple-use-keychain "${HOME}/.ssh/id_enterprise_github"
echo "run 'pbcopy < ~/.ssh/id_enterprise_github.pub' and paste that into Enterprise GitHub"
...
echo "installing latest node version. Node is an alias for latest"
# Create .npmrc and set default to latest version. 'nvm use', 'nvm install', 'nvm exec',
# 'nvm run', 'nvm which' will use the version specified in the .nvmrc file
# if no version is specified on the command line. You can create an .npmrc file
# for every folder where you want to specify a node version.
...
#set finder structure to display folders always on top
#downloads folder as array
